...
Stage_list <- list(stage_1_results, stage_2_results, stage_3_results, stage_4_results, stage_5_results, stage_6_results, stage_7_results, stage_8_results, stage_9_results, stage_10_results, stage_11_results, stage_12_results, stage_13_results, stage_14_results,
...
Teams <- tibble(`Rider type` = c("General", "General", "General", "General", "Sprinter", "Sub"),
                Stiff = c("Pogačar Tadej", "Haig Jack", "Woods Michael", "Ciccone Giulio", "Philipsen Jasper", "Hirschi Marc"),
...
Total_score <- Stage_Scoring(stage_1_results)

#Total_score <- map_dfr(stage_1_results, Stage_Scoring) 
...
a <- 
  Total_score_tidy %>%
  select(Rider, BIB, Total_stage_points)
...
Results <- 
  ...
  mutate(GC_points = replace_na(GC_points, 0),
         ...
         Final_score = Total_stage_points + GC_points) %>% 
  arrange(-Final_score) %>% 
  select(Rider, BIB, GC, Total_stage_points, GC_points, Final_score)
...
Team_rankings <-
  ...
  select(Picked_by, Total_stage_points, `Provisional score`) %>% 
  rename(`Confirmed points` = Total_stage_points) %>% 
...
my_data_table(Rider_rankings %>% 
                select(Rider, `Directeur sportif`, `Provisional score`,
                       `GC points`, `Cumulative stage points`))
